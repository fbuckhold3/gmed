...
#
# Promoted from imslu.ind.dash (2026-09) so both ind.dash and
# imslu.resident.digest can compose it — Fred wanted a single place for
# residents to act, not a link back to ind.dash for this one section. Same
# coupling pattern as every other promotion this round: the 3 REDCap
# helpers (.fe_next_instance/.fe_submit_eval/.fe_submit_pending) read
# app_config directly; everything else (the multi-step form, PGY chart,
# faculty search) was already clean. .fe_submit_pending uniquely needs the
# FAC_TOKEN (separate database from RDM) — now an explicit `fac_token`
# param rather than app_config$fac_token.
#
# rdm_data() is expected in the same shape gmed::load_rdm_for_resident()
# already returns (residents, all_forms$faculty_evaluation, data_dict) —
# callers outside ind.dash can just pass that loader's result directly,
# no new shape to build.
#
# Section 1: Summary strip + PGY year progress chart + faculty name chips
# Section 2: Native multi-step evaluation form (no iframe)
#            Uses a faculty roster reactive (loaded via FAC_TOKEN by the
#            caller). Submits directly to REDCap via the params below.

#' @importFrom shiny NS tagList singleton tags HTML uiOutput div textInput fluidRow column selectInput textAreaInput actionButton moduleServer reactiveVal reactive req renderUI showNotification observeEvent
...
# All 3 now take redcap_url/rdm_token (and, for .fe_submit_pending, fac_token)
# explicitly — were implicit via ind.dash's app_config global before this
# promotion.

.fe_next_instance <- function(resident_id, redcap_url, rdm_token) {
  tryCatch({
    resp <- httr::POST(
      redcap_url,
      body = list(
        token = rdm_token, content = "record",
...
.fe_submit_eval <- function(resident_id, pgy, eval_data, redcap_url, rdm_token) {
  next_inst <- .fe_next_instance(resident_id, redcap_url, rdm_token)
  full_data <- c(
    list(
...
.fe_submit_pending <- function(name, type_code, rot, redcap_url, fac_token) {
  if (!nzchar(fac_token)) stop("FAC_TOKEN not configured")
  next_inst <- tryCatch({
    r <- httr::POST(
      redcap_url,
      body = list(
        token = fac_token, content = "record", action = "export",
  ...
  resp <- httr::POST(
    redcap_url,
    body = list(
      token = fac_token, content = "record", format = "json", type = "flat",
      data = jsonlite::toJSON(rc_df, auto_unbox = TRUE)
    ),
    encode = "form"
  )
...
# Server
# ═════════════════════════════════════════════════════════════════════════════
#' Faculty evaluation module server
#'
#' @param id Module id.
#' @param rdm_data reactive() -> list shaped like gmed::load_rdm_for_resident()
#'   (residents, all_forms$faculty_evaluation, data_dict).
#' @param resident_id reactive() -> record_id (the evaluator).
#' @param faculty_roster_r reactive() -> faculty roster data.frame (fac_name,
#'   fac_div, fac_fell), loaded via FAC_TOKEN by the caller.
#' @param rdm_token,redcap_url RDM REDCap credentials.
#' @param fac_token Faculty database REDCap token — only used for the
#'   "can't find this faculty member" pending-queue path.
#' @export
mod_faculty_eval_server <- function(id, rdm_data, resident_id, faculty_roster_r,
                                    rdm_token, redcap_url, fac_token) {
  moduleServer(id, function(input, output, session) {
    ...
    pgy_info <- reactive({
      ...
      if (is.na(gy)) return(list(pgy = 1L, goal = 25L, grad_yr = NA_real_))
      info <- tryCatch(calculate_pgy_and_period(gy, tp), error = function(e) NULL)
      # `%||%` only substitutes on NULL, not NA — calculate_pgy_and_period()
      # can legitimately return pgy_year = NA (e.g. an unusual/synthetic
      # grad_yr+type combo, confirmed live 2026-09-16 via the "Demo
      # Resident" test account), which used to flow through uncaught into
      # if() conditions downstream ("missing value where TRUE/FALSE
      # needed"). Explicit NA checks here instead of relying on %||% alone.
      pgy_raw <- if (!is.null(info)) info$pgy_year else NA_integer_
      pgy     <- if (is.null(pgy_raw) || is.na(pgy_raw)) 1L else as.integer(pgy_raw)
      goal    <- .FE_GOALS[as.character(pgy)]
      if (is.na(goal)) goal <- 25L
      list(pgy = pgy, goal = as.integer(goal), grad_yr = gy)
    })
    ...
    do_submit <- function() {
      ...
      tryCatch({
        ...
        .fe_submit_eval(rid, info$pgy, ed, redcap_url = redcap_url, rdm_token = rdm_token)
    ...
    observeEvent(input$add_pending, {
      ...
      tryCatch({
        .fe_submit_pending(name, type, rot, redcap_url = redcap_url, fac_token = fac_token)
