...
#
# Promoted from imslu.ind.dash (2026-09) so it can be reused by both
# ind.dash and imslu.resident.digest without duplicating the ERAS field
# list/validation logic. No behavior change from the ind.dash original
# except `redcap_url`/`rdm_token` are now explicit server params (was
# implicit via ind.dash's `app_config` global) and the save call goes
# through `rc_save_instance()` (see redcap_write.R) instead of the local
# `.rc_save()` ind.dash's other modules still use.
#
...
#' @importFrom shiny NS tagList uiOutput renderUI reactiveValues reactiveVal observeEvent req actionButton div tags conditionalPanel selectInput selectizeInput dateInput radioButtons moduleServer reactive
NULL
...
mod_scholarship_entry_ui <- function(id) {
  ns <- shiny::NS(id)
  shiny::tagList(
    shiny::uiOutput(ns("add_button_panel")),
    shiny::uiOutput(ns("add_form_panel"))
  )
}

#' Scholarship entry form module server
#'
#' @param id Module id.
#' @param resident_id reactive() -> record_id to save under.
#' @param existing_data reactive() -> this resident's current scholarship
#'   rows (used for the next repeat instance + most-meaningful cap + the
#'   collection-picker's existing labels).
#' @param edit_req reactive() -> NULL, or list(mode, instance, row) to open
#'   the form in edit / recategorize mode prefilled from `row`.
#' @param redcap_url REDCap API base URL.
#' @param rdm_token REDCap API token (test or prod — caller's choice).
#' @return reactive() -> saved row (data.frame incl. redcap_repeat_instance).
#' @export
mod_scholarship_entry_server <- function(id, resident_id, existing_data,
                                         edit_req = shiny::reactive(NULL),
                                         redcap_url, rdm_token) {
  shiny::moduleServer(id, function(input, output, session) {
    ns <- session$ns

    ss        <- shiny::reactiveValues(save = NULL)
    saved_row <- shiny::reactiveVal(NULL)
    # form_mode: NULL (closed) or list(mode, instance, initial=named list, ref=text)
    form_mode <- shiny::reactiveVal(NULL)
    ...
    shiny::observeEvent(input$btn_add,    open_for("new"))
    shiny::observeEvent(input$btn_cancel, { form_mode(NULL); ss$save <- NULL })
    shiny::observeEvent(edit_req(), {
      r <- edit_req(); shiny::req(r)
      open_for(r$mode, instance = r$instance, row = r$row)
    }, ignoreNULL = TRUE)

    # ── add button (hidden while a form is open) ───────────────────────────────
    output$add_button_panel <- shiny::renderUI({
      if (!is.null(form_mode())) return(NULL)
      shiny::div(class = "mb-3",
        shiny::actionButton(ns("btn_add"), "+ Add Entry", class = "btn btn-sm",
          style = "background:var(--gmed-primary); color:#fff; border:none; padding:4px 14px; font-size:0.85rem;"))
    })

    # ── entry form ─────────────────────────────────────────────────────────────
    output$add_form_panel <- shiny::renderUI({
    ...
    shiny::observeEvent(input$btn_save, {
      ...
      result <- rc_save_instance(resident_id(), "scholarship", inst, fields,
