...
#' Find the next available repeat-instance number for a record+instrument.
#'
#' The same "pull existing instances, take max + 1" pattern used ad hoc
#' across ind.dash's write paths (peer review, faculty eval, questions) —
#' collected here once so new write paths (e.g. the resident digest's
#' attendance entry) don't re-derive it.
#'
#' @inheritParams count_recent_instances
#' @return Integer: 1 if the record has no existing instances of this
#'   instrument, else max(existing instance numbers) + 1.
#' @export
rc_next_instance <- function(rdm_token, redcap_url, record_id, instrument) {
  tryCatch({
    resp <- httr::POST(
      redcap_url,
      body = list(
    ...
    if (httr::status_code(resp) == 200) {
      ...
      if (is.data.frame(dat) && nrow(dat) > 0) {
        rows <- dat[!is.na(dat$redcap_repeat_instrument) &
                      dat$redcap_repeat_instrument == instrument, ]
        if (nrow(rows) > 0) {
          inst <- suppressWarnings(as.numeric(rows$redcap_repeat_instance))
          return(as.integer(max(inst[!is.na(inst)], 0L) + 1L))
        }
      }
    }
    1L
